...
** Backup folder to remote server through SSH                         :rsync:
#+BEGIN_SRC sh
  rsync -e 'ssh' --info=progress2 -a -r /local/path user@remotehost:~/target/path
#+END_SRC
...
** Unzip multi-part zip files                                        :others:
#+BEGIN_SRC sh
  cat test.zip.00{1,2,3} > test.zip
  unzip test.zip
#+END_SRC
