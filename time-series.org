...
#+CAPTION: Generic process of bearing fault detection using vibration signal
...
\begin{equation}
  \sum_t{\frac{\left(res(t) - f(t)\right)^2}{f(t)^2}} \lt \epsilon
\end{equation}
...
\begin{equation}
\hat{x}_{t} = \alpha_{0} + \alpha_{1} x_{t-1} + \alpha_{2} x_{t-2} + \dots + \alpha_{k} x_{t-k} + \epsilon_{t}
\end{equation}
...
*** AutoRegression
\begin{align}
AR(1) &\rightarrow \hat{y}_t = \alpha_1 y_{t-1} + \epsilon_t \\
AR(2) &\rightarrow \hat{y}_t = \alpha_1 y_{t-1} + \alpha_2 y_{t-2} + \epsilon_t \\
AR(p) &\rightarrow \hat{y}_t = \alpha_1 y_{t-1} + \alpha_2 y_{t-2} + \dots + \alpha_p y_{t-p} + \epsilon_t \\
\end{align}
...
*** ARMA(p, q)
\begin{equation}
\hat{y}_t = \phi_1 y_{t-1} + \phi_2 y_{t-2} + \ldots + \phi_p y_{t-p} + \theta_1\epsilon_{t-1}
